...
The CIA editor and embedded public page use a same-origin, source-checked preview-message handshake. Selecting an item or focusing its form field reveals the real public section (including nested disclosures), scrolls only the iframe, and keeps an orange highlight; clicking a preview item selects its form. Tab changes must not reload the iframe or reset the quote scenario. Hidden items and inapplicable promotions fall back to their section with an explicit status message. Preview annotations never appear on the normal public page. Focused navigation tests run with `npx ng test --watch=false --browsers=ChromeHeadless --ts-config=tsconfig.cia-preview-spec.json --include=src/app/pages/philippines/cia-school/cia-content-preview.spec.ts`.
...
3. CIA's employee-editable course, room, local-fee, peak-season, promotion, and quote-rule content lives in a versioned `SchoolContentRevision` JSON document. Published content overrides CIA's database/compiled defaults on the public page; the page, calculator, local-fee table, and generated quote image must continue to use the same published configuration. `cia-content-config.ts` is the fallback/default schema, not a second published source of truth.
...
CELLA Uni Sparta and CELLA Premium share `cella-pricing.ts`, `cella-student-quote.ts`, and the standalone `CellaQuoteCalculatorComponent`; keep these sources synchronized with `SeedCellaPricingAsync`. Each campus route has its own campus-locked calculator and quote image: never expose a cross-campus selector or mix Uni and Premium students in one quote. Each route supports single people and independent 2–20-person groups, with course, room, Sunday start date, enrollment status and promotion per student. Premium also offers the four supplied Family Package combinations as course choices. Their 4/6/8-week prices are exact whole-family package totals that already include registration, course, accommodation and the poster-listed local fees; never add ordinary room, registration, pickup, laundry or local-fee rows again, and do not infer promotion stacking. The 2026/08/30–2027/01/02 low-season offer deducts 100 dollars once per complete four-week eligible room period, never once from both course and room. Premium six-person accommodation uses 499 dollars per four weeks in the window instead of 600, can stack with the 8/12/16-week long-stay deductions of 50/100/150 dollars, and cannot receive the low-season 100-dollar deduction. Social 6+2 and 9+3 offers use actual stays of 8/12 weeks while charging 6/9 weeks of both course and quad accommodation; local fees always use actual weeks. These offers are new-student-only, do not stack with any other promotion, and page/image must retain the complete Xiaohongshu/Douyin obligations and recovery warning. Do not enable the old Christmas/New Year or Sida discount. Optional pickup, laundry and room-deposit references must not be blindly multiplied for groups. All CELLA website and image money labels use Chinese currency names.
...
CIA was the first I.BREEZE-style group-quote pilot; PINES now uses the same shared group-plan and image infrastructure with its own pricing rules. `cia-student-quote.ts` keeps CIA per-student pricing separate; `school-group-quote.ts` aggregates already-calculated local fees and promotions without owning school prices. Default single / 2–20-person modes retain inactive edits but exclude them from totals. Each student has registration date, adult/minor age selection (selected-course tuition in both cases), returning flag and six visa types. Registration is waived for returning students or individually qualified Christmas offers, never twice. Preserve CIA's 40/60/80% short-stay ratios, 2026/2027 price gate, 95% tuition/room rate and individual seasonal/Christmas eligibility. Tourist visas default to59 days with30 available; use actual stay span and existing CIA extension prices. Long-term student/work/SRRV/SIRV provisionally zero SSP, SSP E-CARD, ACR and extensions, retaining adviser-confirmation notes. ARP is300 pesos once on first tourist extension and once for long-term visas (not exempt). Deposit is2,500 pesos per person; pickup remains a price reference pending actual arrangements. Both show renminbi estimates. Group images use “CIA N人报价”, student-labeled course/accommodation names, merged same-type discounts with eligibility preserved, and identical local-fee notes. CIA opts into `expandTotalNote` so per-student IAU50-dollar warnings remain complete beside the school total, below the conversion rate; other schools keep their existing total layout. Further school migrations await user acceptance.

PINES uses `pines-student-quote.ts` with the CIA quote form, five-column local-fee table and full-detail image presentation, but keeps all PINES pricing and rules independent. It supports single and 2–20-person quotes, independent multi-row course/accommodation plans, Sunday starts, Saturday ends, 2–24 whole weeks per row, age, 30/59-day tourist-visa choice, returning-student information and optional Manila/Clark pickup per student. Public course and room catalogs, quote selectors, webpage payment rows and generated-image payment rows are separated into Main Campus and IELTS Campus groups; within each campus, room choices are sorted naturally by capacity/type from single rooms through larger shared rooms rather than by price, and overlapping course and accommodation rows from different campuses block calculation and image export. IELTS Campus offers5B Solo at650 dollars and single room C at970 dollars per four weeks without comparing those descriptions to Main Campus. Two weeks cost65% and three weeks85% of the four-week course and room prices; four weeks and above are proportional. The Main Campus family 2–3-person room is780 dollars per person per four weeks. PINES promotions stack: the 2026 eight-week peak season is June28–August22 and the aligned 2027 estimate is June27–August21 at40 dollars per overlapping course week; eligible non-peak complete four-week study blocks ending by2026/12/31 deduct150 dollars;12+ weeks deduct100 dollars;16/20/24 weeks deduct an additional100/150/200 dollars. All Sida applicants receive the100-dollar registration waiver. Apply fixed course/room deductions first, then calculate the Sida95% rate on the remaining course/room amount. Local fees use the student's full stay and selected30/59-day initial visa; ACR and the6,210-peso 30-day extension begin with the first required extension. Water is3,000 pesos per four accommodation weeks. School deposit4,000 pesos per four accommodation weeks, refundable room deposit4,000 pesos per person and laundry1,200 pesos are reference rows excluded from the local-fee and renminbi totals. Prefer live exchange rates and fall back to9 pesos per yuan. Keep page, image, and `SeedPinesPricingAsync` synchronized.
...
Baguio JIC uses `jic-pricing.ts` and `jic-student-quote.ts` for its confirmed Challenger/Premium course, room, promotion and local-fee rules; keep them synchronized with `SeedJicPricingAsync`. The user confirmed the supplied 2025 four-week USD catalog remains the current 2026 price basis. The public course and accommodation catalogs are visibly separated into Challenger and Premium groups; course `displayName` values are bilingual and are shared by the public table, quote selector and quote image while `name` remains the stable database lookup value. Quotes start at four weeks and offer 4/6/8/12/16/20/24-week rows; six weeks is 1.5 times the four-week course, room, management, utilities, laundry and optional Challenger-elective amount. Each student independently selects a campus, course/room periods, registration date, 30/59-day tourist visa, pickup, returning status and extension status; one JIC quote supports single people and independent 2–20-person groups. Promotions use the sheet's `ARRIVAL DATES` as Sunday arrival dates: 2026/2027 low-season and holiday discounts do not apply to in-school extensions, while the 12/16/20/24-week long-term tiers may apply to extensions and stack. BESA requires registration during 2026/04/01–06/30 and arrival during 2026/08/23–12/13 and counts only complete four-week blocks. All students studying 12 weeks or more receive the Sida 100-dollar registration waiver; returning and extension students also do not repay registration. The only confirmed peak surcharge is 40 dollars per overlapping course week during 2026/06/28–08/22. Applied JIC surcharges and promotions appear as calculated school-payment rows with their applicable conditions; do not repeat the full promotion catalog in the image footer, which contains only quote-wide rules, currency context and final confirmation. Single-campus quote images use the matching campus photo (Challenger/Main or Premium); a mixed-campus group uses the general JIC campus hero. Local fees use 7,800-peso SSP, 4,500-peso SSP E-CARD, 4,000-peso ACR once on first extension, 4,940 pesos per additional 30 visa days, one 1,900-peso textbook set per started eight course weeks, a 200-peso student ID, proportional four-week accommodation fees, selected 3,000-peso Manila/Clark pickup, and a separately excluded refundable 3,000-peso deposit. Webpage and full-detail image share the same calculated rows and notes.
