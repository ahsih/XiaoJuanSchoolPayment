...
import { cpils2027Eligible, cpilsMergedCourses, cpilsMonday, cpilsMoney, cpilsPolicyError, cpilsPriceYear, cpilsRowPrice } from './cpils-pricing';
...
interface CourseFee { id: string; name: string; tuition: number; tuition2027: number; suitable: string; }
...
export class CpilsSchoolDetailComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  private createStudent(): CpilsStudentQuote {
    const student = { selectedAgeGroup: 'adult', visaType: 'tourist59', returningStudent: false, pickupSelected: false, selectedRegistrationDate: '2026-09-29' } as CpilsStudentQuote;
    student.quotePlan = new SchoolQuotePlan('general-esl', 'regular-quad', '2027-01-03', this.weekOptions,
      ...
      (kind, row) => cpilsRowPrice(this.contentConfig, student, kind, row), 24, true, false);
    return student;
  }
  ...
  readonly courses: CourseItem[] = [
    ...
    { name: '雅思课程 / 雅思预备课程 / 雅思保证班', type: '雅思备考', lessons: '雅思预备课程以1:1为主；雅思课程为4堂1:1 + 5堂大团体 + 3堂强制自修', suitable: '雅思保证班提供固定8周或12周方案；2027年雅思预备课程价格须确认。' },
    ...
    { name: 'Business / PMC演讲课程', type: '商务与演讲', lessons: 'Business为4堂1:1 + 2堂1:4 + 1堂1:12 + 2堂大团体；PMC增加口语与强制自修', suitable: '2027年提供4周或8周方案。' },
    { name: 'ESL Premium / TESOL / 职业英语', type: '其他课程', lessons: '2027年价目表另含医学英语、酒店与服务业、咖啡师与饮品、打工度假及实习项目。', suitable: '课程安排、入学要求和实习资格请向顾问确认。' },
  ];

  courseFees: CourseFee[] = this.initialContent.courses.map(item => ({ id: item.id, name: item.name, tuition: item.tuition, tuition2027: item.tuition2027, suitable: [item.schedule, item.suitable].filter(Boolean).join('；') }));
  roomFees: RoomFee[] = this.initialContent.rooms.map(item => ({ id: item.id, name: item.name, fee: item.fee, note: item.note }));
  priceTableYear: 2026 | 2027 = 2027;
  courseTablePrice(id: string): string {
    const item = this.contentConfig.courses.find(course => course.id === id);
    if (!item) return '待确认';
    const exact = this.priceTableYear === 2027 ? item.feeByWeeks2027 : item.feeByWeeks;
    if (id.includes('guarantee')) return Object.entries(exact ?? {}).map(([weeks, price]) => `${weeks}周 ${this.formatUsd(price)} 美元`).join('；') || '待确认';
    const price = this.priceTableYear === 2027 ? item.tuition2027 : item.tuition;
    return price > 0 ? `4周 ${this.formatUsd(price)} 美元` : '待确认';
  }
  roomTablePrice(id: string): string {
    const item = this.contentConfig.rooms.find(room => room.id === id);
    const price = this.priceTableYear === 2027 ? item?.fee2027 : item?.fee;
    return price ? `${this.formatUsd(price)} 美元` : '待确认';
  }
  tableShortPrices(kind: 'course' | 'room', id: string): string {
    const item = (kind === 'course' ? this.contentConfig.courses : this.contentConfig.rooms).find(option => option.id === id);
    const prices = this.priceTableYear === 2027 ? item?.feeByWeeks2027 : item?.feeByWeeks;
    const entries = [1, 2, 3].filter(weeks => prices?.[weeks] !== undefined).map(weeks => `${weeks}周 ${this.formatUsd(prices![weeks])}美元`);
    return entries.length ? entries.join('；') : '1–3周价格未提供';
  }
  get currentPromotions() { return this.promotionRules.filter(rule => rule.enabled && (rule.id.endsWith('-2027') || rule.id === 'cpils-sida' || rule.id === 'cpils-returning-registration')); }
  studentPriceNote(student: CpilsStudentQuote): string {
    const year = cpilsPriceYear(this.contentConfig, student);
    return year === 'unconfirmed' ? '所选日期价格待顾问确认' : `${year}年价格 · 开课日 ${cpilsMonday(student.quotePlan.courses[0]?.startDate ?? '')}${year === 2026 && student.quotePlan.startDate >= '2027-01-01' ? '；不适用2027年学校优惠' : ''}`;
  }
  ...
  readonly faqs: FaqItem[] = [
    ...
    { question: 'CPILS优惠有哪些？', answer: '2027年有学费及住宿费5%奖学金、无对外窗单／双人房优惠，以及雅思和托业官方考试赠送。另有思达优惠；具体报名日、开课日、课程和房型条件请查看上方优惠板块，计算器会显示本次适用项目。' },
  ...
  readonly sideNav: SideNavItem[] = [
    ...
    { label: '2027年优惠', target: 'promotions', icon: 'local_offer' },
  ...
  readonly mobileAnchors: SideNavItem[] = [
    ...
    { label: '优惠', target: 'promotions', icon: 'local_offer' },
  ...
  private applyContentConfig(value: CiaContentConfig): void {
    ...
    this.courseFees = courses.map(item => ({ id: item.id, name: item.name, tuition: item.tuition, tuition2027: item.tuition2027, suitable: [item.schedule, item.suitable].filter(Boolean).join('；') }));
  ...
  private loadSchoolContent(): void {
    this.schoolService.getSchools({ name: this.pricingSchoolSearchName }).pipe(
      ...
      else this.applyContentConfig(this.initialContent);
  ...
  get selectedCourse(): CourseFee | undefined { return this.courseFees.find((course) => course.id === this.selectedCourseId) ?? this.courseFees[0]; }
  get selectedRoom(): RoomFee | undefined { return this.roomFees.find((room) => room.id === this.selectedRoomId) ?? this.roomFees[0]; }
  ...
  private offSeasonRule(row: QuotePlanRow): CiaPromotionRule | undefined {
    if (cpilsMonday(row.startDate) >= '2027-01-01') return undefined;
  ...
  private studentOffSeasonEligibleBase(student: CpilsStudentQuote): number {
    if (cpils2027Eligible(this.contentConfig, student, this.promotionRule('cpils-scholarship-2027'))) return this.studentBase(student) * this.sidaDiscountRate;
  ...
  private studentOffSeasonDiscount(student: CpilsStudentQuote): number {
    const scholarship = this.promotionRule('cpils-scholarship-2027');
    if (cpils2027Eligible(this.contentConfig, student, scholarship)) return this.roundMoney(this.studentBase(student) * this.sidaDiscountRate * scholarship!.discountValue / 100);
    if (cpilsMonday(student.quotePlan.courses[0]?.startDate ?? '') >= '2027-01-01') return 0;
  ...
  private studentNoWindowDiscount(student: CpilsStudentQuote): number {
    if (cpilsMonday(student.quotePlan.courses[0]?.startDate ?? '') >= '2027-01-01') {
      const rule = this.promotionRule('cpils-no-window-2027');
      if (!cpils2027Eligible(this.contentConfig, student, rule)) return 0;
      const weeks = student.quotePlan.rooms.filter(row => rule!.eligibleRoomIds?.includes(row.optionId)).reduce((sum, row) => sum + row.weeks, 0);
      return Math.floor(weeks / Math.max(1, rule!.incrementWeeks ?? 4)) * rule!.discountValue;
    }
  ...
  private studentPeakWeeks(student: CpilsStudentQuote) {
    const ranges = this.contentConfig.quoteSettings.peakSeasonRanges.filter(range => range.enabled);
    return student.quotePlan.weekStarts(student.quotePlan.courses).filter(week => ranges.some(range => week <= Date.parse(range.end) && week + 6 * 86400000 >= Date.parse(range.start))).length;
  }
  ...
  private studentRegistration(student: CpilsStudentQuote) { return student.returningStudent ? 0 : this.registrationFee; }
  ...
  get examBenefitText(): string {
    return this.currentPromotions.filter(rule => rule.id.includes('-exam-')).map(rule => rule.description).join('；');
  }

  private studentExamBenefits(student: CpilsStudentQuote): string[] {
    if (cpilsMonday(student.quotePlan.courses[0]?.startDate ?? '') >= '2027-01-01') {
      const ielts = this.promotionRule('cpils-ielts-exam-2027'), toeic = this.promotionRule('cpils-toeic-exam-2027');
      const courses = cpilsMergedCourses(student.quotePlan);
      const ieltsEligible = cpils2027Eligible(this.contentConfig, student, ielts) && courses.some(row => ielts!.eligibleCourseIds?.includes(row.optionId) && row.weeks >= ielts!.minimumCourseWeeks);
      const toeicWeeks = courses.filter(row => toeic?.eligibleCourseIds?.includes(row.optionId)).reduce((sum, row) => sum + row.weeks, 0);
      return [
        ...(ieltsEligible ? [`雅思课程一次注册12周及以上：赠${ielts!.discountValue}次雅思官方考试`] : []),
        ...(cpils2027Eligible(this.contentConfig, student, toeic) && toeicWeeks >= toeic!.minimumCourseWeeks ? [`托业课程：赠${Math.min(6, Math.floor(toeicWeeks / Math.max(1, toeic!.incrementWeeks ?? 4)) * toeic!.discountValue)}次官方考试`] : []),
      ];
    }
    return cpilsMergedCourses(student.quotePlan).flatMap(row => {
  ...
  get applicableExamBenefits(): string[] {
    return this.activeStudents.flatMap((student, index) => this.studentError(student) ? [] : this.studentExamBenefits(student).map(note => this.quoteMode === 'group' ? `学生${index + 1}：${note}` : note));
  }
  ...
  get quoteError(): string {
    ...
    const index = this.activeStudents.findIndex(student => !!this.studentError(student));
    return index < 0 ? '' : `${this.quoteMode === 'group' ? `学生${index + 1}：` : ''}${this.studentError(this.activeStudents[index])}`;
  }
  studentError(student: CpilsStudentQuote): string { return student.quotePlan.error || cpilsPolicyError(this.contentConfig, student) || (!this.visaOptions.some(option => option.value === student.visaType) ? '请选择有效的签证类型。' : ''); }
  ...
  private visaExtensionCountFor(student: CpilsStudentQuote) { return this.isLongTermVisa(student) ? 0 : Math.max(0, Math.ceil((student.quotePlan.stayWeeks - (student.visaType === 'tourist30' ? 4 : 8)) / 4)); }
  private extensionDate(student: CpilsStudentQuote, index = 0): string {
    const weeks = (student.visaType === 'tourist30' ? 4 : 8) + index * 4;
    const start = student.quotePlan.date(student.quotePlan.startDate);
    return start === null ? '' : new Date(start + weeks * 7 * 86400000).toISOString().slice(0, 10);
  }
  private datedFee(rule: CiaLocalFeeRule, date: string): CiaLocalFeeRule {
    if (!rule.futureEffectiveStart || date < rule.futureEffectiveStart) return rule;
    return { ...rule, name: rule.futureName ?? rule.name, amount: rule.futureAmount ?? rule.amount,
      periodWeeks: rule.futurePeriodWeeks ?? rule.periodWeeks, rounding: rule.futureRounding ?? rule.rounding,
      rates: rule.futureRates ?? rule.rates, note: rule.futureNote ?? rule.note };
  }
  private visaExtensionTotalFor(student: CpilsStudentQuote) {
    ...
    return rule ? Array.from({ length: count }, (_, index) => { const dated = this.datedFee(rule, this.extensionDate(student, index)); return dated.rates?.[index] ?? dated.rates?.at(-1) ?? dated.amount; }).reduce((sum, value) => sum + value, 0) : 0;
  }
  ...
  private studentLocalFees(student: CpilsStudentQuote): SchoolLocalFee[] {
    const plan = student.quotePlan, extensions = this.visaExtensionCountFor(student), longTerm = this.isLongTermVisa(student);
    const visaNote = `${this.visaLabel(student)}相关费用暂按0估算，是否免收请由顾问向学校确认，以学校最新政策为准。`;
    const rows: SchoolLocalFee[] = [];
    for (const base of this.localFeeRules.filter(rule => rule.enabled && rule.includeInTotal)) {
      const extensionItem = ['acr-i-card', 'arp', 'visa-extension'].includes(base.id);
      const date = extensionItem && !longTerm ? this.extensionDate(student) : plan.startDate;
      const rule = this.datedFee(base, date);
      let quantity = 1, total: number | undefined, note = rule.note;
      let unitLabel = `${this.formatPhp(rule.amount)}／次`;
      if (rule.billingRule === 'per-accommodation-period' || rule.billingRule === 'per-course-period') {
        const weeks = rule.billingRule === 'per-course-period' ? plan.courseWeeks : plan.roomWeeks;
        quantity = weeks / Math.max(1, rule.periodWeeks ?? 4);
        if (rule.rounding === 'ceil') quantity = Math.ceil(quantity);
        unitLabel = `${this.formatPhp(rule.amount)}／${rule.periodWeeks ?? 4}周`;
        // Only the management rate changes during an existing stay. Split on the
        // actual effective date, without moving January 1 to a different weekday.
        if (base.id === 'management' && base.futureEffectiveStart && plan.startDate < base.futureEffectiveStart && plan.endDate >= base.futureEffectiveStart) {
          const boundary = Date.parse(base.futureEffectiveStart);
          const days = [...plan.days(plan.rooms)];
          const before = days.filter(day => day < boundary).length, after = days.length - before;
          const future = this.datedFee(base, base.futureEffectiveStart);
          quantity = plan.roomWeeks;
          total = this.roundMoney(before / 7 * base.amount / (base.periodWeeks ?? 4) + after / 7 * future.amount / (future.periodWeeks ?? 4));
          unitLabel = `${this.formatPhp(base.amount / (base.periodWeeks ?? 4))}→${this.formatPhp(future.amount / (future.periodWeeks ?? 4))}／周`;
          note = future.note;
        }
      }
      if (base.id === 'acr-i-card') { quantity = !longTerm && extensions > 0 ? 1 : 0; note = longTerm ? visaNote : `按${this.visaLabel(student)}预估；${rule.note}`; }
      if (base.id === 'arp') { quantity = longTerm || extensions > 0 ? 1 : 0; if (longTerm) note = `${this.visaLabel(student)}仍预留一次；${rule.note}`; }
      if (base.id === 'visa-extension') {
        quantity = extensions; total = this.visaExtensionTotalFor(student); unitLabel = '按所选签证及学校续签方案';
        note = longTerm ? visaNote : extensions ? `按${this.visaLabel(student)}预估，本次${extensions}次；${rule.note}` : `按${this.visaLabel(student)}预估，本次无需续签。`;
      }
      if (longTerm && rule.waiveForLongTermVisa) { quantity = 0; total = 0; note = visaNote; }
      rows.push({ item: rule.name, unitLabel, quantity, total: total ?? this.roundMoney(rule.amount * quantity), note });
    }
    return rows;
  }
  ...
  get excludedLocalFees(): LocalFee[] {
    const references = this.activeStudents.map(student => ({ localFees: this.localFeeRules.filter(rule => rule.enabled && !rule.includeInTotal && (rule.id !== 'crtv' || student.quotePlan.endDate >= '2027-01-01')).map(base => {
      const rule = this.datedFee(base, student.quotePlan.startDate);
      const quantity = rule.id === 'pickup' ? Number(student.pickupSelected) : rule.id === 'deposit' ? 1 : 0;
      return { item: rule.name, unitLabel: this.formatPhp(rule.amount), quantity, total: rule.amount * quantity, note: `${rule.note}不计入学杂费合计。` };
    }) }));
    return groupLocalFees(references).map(fee => ({ item: fee.item, amount: fee.quantity ? this.formatPhp(fee.total) : fee.unitLabel, quantity: fee.quantity, total: fee.total, note: fee.note, excluded: true }));
  }
  ...
  private studentPaymentLines(student: CpilsStudentQuote): SchoolPaymentLine[] {
    ...
    const scholarship = this.promotionRule('cpils-scholarship-2027');
    const offSeasonRule = cpils2027Eligible(this.contentConfig, student, scholarship) ? scholarship : [...student.quotePlan.courses, ...student.quotePlan.rooms].map(row => this.offSeasonRule(row)).find((rule): rule is CiaPromotionRule => !!rule);
    const noWindowRule = this.promotionRule(cpilsPriceYear(this.contentConfig, student) === 2027 ? 'cpils-no-window-2027' : 'cpils-no-window');
    ...
    // The full tier catalogue belongs in the promotion card. An applied quote
    // needs only this room condition; preserve explicitly edited descriptions.
    const defaultNoWindow = this.initialContent.quoteSettings.promotions.find(rule => rule.id === 'cpils-no-window-2027');
    const noWindowNote = noWindowRule?.id === defaultNoWindow?.id && noWindowRule?.description === defaultNoWindow?.description
      ? '无对外窗单人／双人房，窗户面向走廊；符合2027年优惠报名及开课条件，可与其他优惠同享。'
      : noWindowRule?.description ?? '';
    return [
      ...
      ...(noWindow && noWindowRule ? [{ icon: '房', label: noWindowRule.name, value: -noWindow, note: noWindowNote, promotionKey: noWindowRule.id }] : []),
  ...
  get quoteImageData() {
    ...
    const paymentItems: QuoteImagePaymentItem[] = [
      ...
      ...(['课', '宿'] as const).flatMap(icon => this.activeStudents.flatMap((student, index) => student.quotePlan.paymentItems().filter(item => item.icon === icon).map(item => ({
        ...
        note: [item.note, icon === '课' ? this.studentPriceNote(student) : '', icon === '课' ? settings.paymentNotes.course : settings.paymentNotes.accommodation].filter(Boolean).join('；'),
      })))),
      ...groupPaymentLines(this.activeStudents.map(student => ({ paymentLines: this.studentPaymentLines(student) })), true),
    ...
    const quote = buildPhilippinesDetailedQuote({
      ...
      paymentItems,
      localFeeItems: this.includedLocalFees.map((fee) => { const id = this.localFeeRules.find(rule => rule.name === fee.item)?.id ?? ''; return { label: fee.item, unit: fee.amount, quantity: String(fee.quantity), amount: this.formatPhp(fee.total), note: fee.note }; }),
      ...
      optionalFeeItems: this.excludedLocalFees.map(fee => { const id = this.localFeeRules.find(rule => rule.name === fee.item)?.id ?? ''; return { label: fee.item, amount: fee.amount, cnyAmount: fee.total ? `约人民币 ${Math.round(fee.total / this.phpPerCny).toLocaleString('zh-CN')} 元` : '', note: fee.note }; }),
  ...
  previewFeeId(name: string) { return this.localFeeRules.find(item => name.endsWith(item.name) || (!!item.futureName && name.endsWith(item.futureName)))?.id ?? 'local-fees'; }
  ...
  private roundMoney(value: number): number { return cpilsMoney(value); }
